...
static unsigned char *generate_data(int width, int height, int depth) {
  unsigned char *data = ( unsigned char * )malloc(sizeof(unsigned char) * width * height * depth);
...
static unsigned char base_dir[] = "./data";
...
static void create_dataset(int num, int width, int height, int depth) {
   char dir_name[1024];

printf("Generating dataset %d\n", num);
  ...
  unsigned char *output_data = ( unsigned char * )calloc(sizeof(unsigned char), width * height * depth);
