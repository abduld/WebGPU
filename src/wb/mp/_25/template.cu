...
static void s2g_gpu_scatter(int *in, int *out, int len) {
  ...
  const int numBlocks = (len - 1) / numThreadsPerBlock + 1;
  s2g_gpu_scatter_kernel << < numBlocks, numThreadsPerBlock >>> (in, out, len);
}

static void s2g_gpu_gather(int *in, int *out, int len) {
  const int numThreadsPerBlock = 512;
  const int numBlocks = (len - 1) / numThreadsPerBlock + 1;
  s2g_gpu_gather_kernel << < numBlocks, numThreadsPerBlock >>> (in, out, len);
}
